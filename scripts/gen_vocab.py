...
def update_vocab(path, vocab, vocab_edge, vocab_node):
    ...
    with open(path, 'rb') as fr:
        ...
        for line in lines:
            ...
            words += [x for x in line.lower().strip().split() if x[0] != ':']
            words_edge += [x for x in line.lower().strip().split() if x[0] == ':']
            words_node += [x for x in line.lower().strip().split() if re.search('_[0-9]+', x) != None or x == 'num_unk']
        update(words, vocab)
